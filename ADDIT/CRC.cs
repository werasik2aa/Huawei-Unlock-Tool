...
namespace HuaweiUnlocker
{
    public class CRC
    {
        ...
        private static string FlipToString(string singleByte)
        {
            if (singleByte == "0")
                return "0";
            if (singleByte == "1")
        ...
        public static string ReverseHex(string value)
        {
            ...
            value.Replace(" ", ""); //remove all spacebars

            StringBuilder builder = new StringBuilder();
            for (int i = 0; i <= value.Length - 2; i += 2)
                builder.Append(new string(value.Substring(i, 2).Reverse<char>().ToArray<char>()));

            return HexStringFormatter(Microsoft.VisualBasic.Strings.StrReverse(builder.ToString()));
        }
