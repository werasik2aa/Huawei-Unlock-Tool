...
namespace HuaweiUnlocker.HW_COM
{
    public class ComFlash
    {
        ...
        public void Write(string path, int address, Action<int> reportProgress = null)
        {
            ...
            while (length > MAX_DATA_LEN)
            {
                ...
                if (n % (nframes > 250 ? 10 : 3) == 0) reportProgress?.Invoke((int)(100f * n / nframes));
            }
        ...
        private void SendFrame(byte[] data)
        {
            ...
            if (_ack != 0xAA) throw new Exception(string.Format("ACK is invalid! ACK={0:X2}; Excepted={1:X2}", _ack, 0xAA));
        }
    }
}
