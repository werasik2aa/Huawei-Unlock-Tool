...
namespace HuaweiUnlocker.DIAGNOS
{
    public class CRC : HashAlgorithm
    {
        ...
        private static readonly ushort[] CRCqualcomm = new ushort[] {
        ...
        public static ushort ComputeChecksum(byte[] bytes)
        {
            ...
            for (var i = 0; i < bytes.Length; ++i)
                crc = ((crc >> 8) ^ CRCqualcomm[(crc ^ bytes[i]) & 0xFF]) & 0xFFFF;
        ...
        public static byte[] GetBufferWithCRC(byte[] data, int count)
        {
            ushort crc = 0;
        ...
        public static string BytesToHexString(byte[] byteInput)
        {
            try
            {
                return string.Join(string.Empty, byteInput.Select(x => x.ToString("X2")));
            }
            catch (Exception e)
        ...
        public static string HexToIMEI(string hexInput)
        {
            ...
            StringBuilder builder = new StringBuilder();
            for (int i = 1; i <= str.Length - 2; i += 2) builder.Append(new string(str.Substring(i, 2).Reverse<char>().ToArray<char>()));
