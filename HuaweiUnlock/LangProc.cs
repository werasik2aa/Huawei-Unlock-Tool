    ...
    public static class LangProc
    {
        ...
        public static bool SyncRUN(string command, string subcommand)
        {
            ...
            while ((outtext = p.StandardOutput.ReadLine()) != null)
            {
                ...
                else if (outtext.StartsWith("SerialNumber"))
                    DeviceInfo.SerialNum = outtext.Split(' ')[1];
                else if (outtext.StartsWith("MSM_HW_ID"))
                    DeviceInfo.HWID = outtext.Split(' ')[1];
                else if (outtext.StartsWith("OEM_PK_HASH"))
                    DeviceInfo.PK_HASH = outtext.Split(' ')[1];
                else if (outtext.StartsWith("SBL SW Version"))
        ...
        public static bool isError(string i)
        {
            ...
            if (i.Contains("failed") || i.Contains("error") || i.Contains("error setting com port timeouts") || i.Contains("fail") || i.Contains("status: 2") || i.Contains("failed to write hello response back to device")) return true;
            return false;
        }
        ...
        public static Dictionary<string, Partition> GET_GPT_FROM_FILE(string GPT_File, int block_size)
        {
            ...
            GPT_Struct gpt_startadress = new GPT_Struct(0x48, 8, string.Empty);
            GPT_Struct max_gpt_blocks = new GPT_Struct(0x50, 4, string.Empty);
            GPT_Struct record_length = new GPT_Struct(0x54, 4, string.Empty);
            ...
            GPT_Struct block_startadress = new GPT_Struct(0x20, 8, string.Empty);
            GPT_Struct block_endadress = new GPT_Struct(0x28, 8, string.Empty);
            GPT_Struct block_name = new GPT_Struct(0x38, 72, string.Empty);
